...
export function NadaProvider({ children }: { children: React.ReactNode }) {
  ...
  // Persist after hydration so we don't overwrite stored state with INITIAL_STATE on mount.
  useEffect(() => {
  ...
  return (
    <Ctx.Provider value={{ state, hydrated, intercept, reset }}>{children}</Ctx.Provider>
  );
}
